    ...
    def do_create(self, args):
        ...
        for parameter in args_list[1:]:
            ...
            if (param_value[0] == '"' and param_value[-1] != '"'):
                return
            if (param_value[0] == '"' and param_value[-1] == '"'):
                new_instance.__dict__[param_name] = param_value[1:-1].\
                        replace('_', ' ')
            elif ('.' in param_value):
                new_instance.__dict__[param_name] = float(param_value)
            else:
                new_instance.__dict__[param_name] = int(param_value)
        storage.save()
