#!/usr/bin/python3
# script that that deletes out-of-date archives
...
def do_deploy(archive_path):
    ...
    if result.failed:
        return False
    uncompressed_folder ="/data/web_static/releases/{}".format(archive_path.split("/")[-1].split(".")[0])
...
def deploy():
    ...
    if (not exists(out)):
    	return False
    return do_deploy(out)

def do_clean(number=0):
    """Deletes out-of-date archives"""
    archives = local("ls -1t versions", capture=True)
    archive_names = archives.split("\n")
    n = int(number)
    if (n ==0 or n == 1):
        n = 1
    for i in archive_names[n:]:
        local("rm versions/{}".format(i))
    remote_dir = run("ls -1t /data/web_static/releases")
    remote_dir_names = remote_dir.split("\n")
    for folder in remote_dir_names[n:]:
        if folder is 'test':
            continue
